		...
		<tr>
			<td>
				<table border = 1 class="table1">
					...
					$result = mysqli_query($con,"SELECT * FROM order_info JOIN user_info ON order_info.U_id = user_info.U_id JOIN tailor_info ON order_info.Tailor_id = tailor_info.Tailor_id JOIN design_info ON order_info.Design_id = design_info.Design_id WHERE tailor_info.Tailor_id = '$tid' AND order_info.Order_Status = 'Pending'");
					$no = 1;
					...
					while ($res = mysqli_fetch_array($result)) {
						...
						<?php
							echo "<td><button type='submit' name='finish' formaction='pendingOrder.php?id=$oid' class='button'>Finish</button></td>";						
                    	echo "</tr>";
                    	$no+=1;
